"""
Simple SSS Provident Fund / MPF contribution calculator.
"""
import pandas as pd
    ...
    while current_age < retirement_age + annuity_years:
        for month in range(1, 13):
            ...
            data.append({
                ...
                'Investment Income (Annuity) (₱)': f"{investment_income:,.2f}" if current_age > retirement_age or (current_age == retirement_age and month > 1) else 0,
                'Management Fee (Annuity) (₱)': f"{management_fee:,.2f}" if current_age > retirement_age or (current_age == retirement_age and month > 1) else 0,
