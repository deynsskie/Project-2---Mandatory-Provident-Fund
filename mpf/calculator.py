...
def calculate(age, MSC, annuity_years):
    ...
    # Trackers for aggregate metadata
    ...
    while current_age < retirement_age + annuity_years:
        for month in range(1, 13):
            ...
            # Accumulate metrics
            total_investment_income += investment_income
    ...
    # Pack final calculated values into summary dictionary
    summary_metadata = {
        'Starting Age': f"{age}",
        'MSC (₱)': f"{MSC:,.2f}",
        'Annuity Years': f"{annuity_years}",
        'TAAV at Retirement (₱)': f"{taav:,.2f}",
        'Monthly Pension (₱)': f"{monthly_pension:,.2f}", 
        'Total Benefits Claimed (₱)': f"{total_benefits_claimed:,.2f}",
        'Total Investment Income (₱)': f"{total_investment_income:,.2f}", 
        'Total Management Fees (₱)': f"{total_management_fees:,.2f}" 
    }

    return monthly_pension, total_benefits_claimed, taav, df, summary_metadata
